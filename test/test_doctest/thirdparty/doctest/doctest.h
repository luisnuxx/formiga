...
#define DOCTEST_FORBIT_EXPRESSION(op)                                                              \
    ...
    struct DOCTEST_INTERFACE TestCase
    {
        ...
        funcType m_test;    // a function pointer to the test_catch2 case
        String m_full_name; // contains the name (only for templated test_catch2 cases!) + the template type
        const char* m_name;       // name of the test_catch2 case
        const char* m_type;       // for templated test_catch2 cases - gets appended to the real name
        const char* m_test_suite; // the test_catch2 suite in which the test_catch2 was added
        const char* m_description;
        bool        m_skip;
        bool        m_may_fail;
        bool        m_should_fail;
        ...
        // fields by which uniqueness of test_catch2 cases shall be determined
        const char* m_file; // the file in which the test_catch2 was registered
        unsigned    m_line; // the line where the test_catch2 was registered
        int m_template_id; // an ID used to distinguish between the different versions of a templated test_catch2 case
...
// registers the test_catch2 by initializing a dummy var with a function
...
// for grouping tests in test_catch2 suites by using code blocks
...
// this is here to clear the 'current test_catch2 suite' for the current translation unit - at the top
...
namespace detail
{
    ...
    // 'Invalid read of size 4' when the test_catch2 suite len (with '\0') is not a multiple of 4
    ...
    struct ContextState : TestAccessibleContextState //!OCLINT too many fields
    {
        ...
        unsigned first; // the first (matching) test_catch2 to be executed
        unsigned last;  // the last (matching) test_catch2 to be executed

        int  abort_after;           // stop tests after this many failed assertions
        int  subcase_filter_levels; // apply the subcase filters for the first N levels
        bool case_sensitive;        // if filtering should be case sensitive
        bool exit;         // if the program should be exited after the tests are ran/whatever
        bool duration;     // print the time duration of each test_catch2 case
        ...
        bool no_skip;      // don't skip test_catch2 cases which are marked to be skipped
...
namespace doctest_detail_test_suite_ns
{
// holds the current test_catch2 suite
...
namespace detail
{
    ...
    TestCase& TestCase::operator*(const char* in) {
        m_name = in;
        // make a new name with an appended type for templated test_catch2 case
        if(m_template_id != -1) {
    ...
    // sets the current test_catch2 suite
...
#include <sys/sysctl.h>
    ...
    void printHelp() {
        ...
        std::printf(" -ltc, --list-test_catch2-cases               lists all matching tests by name\n");
        std::printf(" -lts, --list-test_catch2-suites              lists all matching test_catch2 suites\n\n");
        // ========================================================================================= << 79
        DOCTEST_PRINTF_COLORED("[doctest] ", Color::Cyan);
        std::printf("The available <int>/<string> options/filters are:\n\n");
        std::printf(" -tc,  --test_catch2-case=<filters>           filters     tests by their name\n");
        std::printf(" -tce, --test_catch2-case-exclude=<filters>   filters OUT tests by their name\n");
        ...
        std::printf(" -ts,  --test_catch2-suite=<filters>          filters     tests by their test_catch2 suite\n");
        std::printf(" -tse, --test_catch2-suite-exclude=<filters>  filters OUT tests by their test_catch2 suite\n");
        ...
        std::printf(" -f,   --first=<int>                   the first test_catch2 passing the filters to\n");
        std::printf("                                       execute - for range-based execution\n");
        std::printf(" -l,   --last=<int>                    the last test_catch2 passing the filters to\n");
        std::printf("                                       execute - for range-based execution\n");
        ...
        std::printf(" -d,   --duration=<bool>               prints the time duration of each test_catch2\n");
        ...
        std::printf(" -ns,  --no-skip=<bool>                don't skip test_catch2 cases marked as skip\n");
    ...
    void printSummary() {
        ...
        if(p->count || p->list_test_cases) {
            DOCTEST_PRINTF_COLORED("[doctest] ", Color::Cyan);
            std::printf("unskipped test_catch2 cases passing the current filters: %u\n",
            ...
            std::printf("unskipped test_catch2 cases passing the current filters: %u\n",
                        p->numTestsPassingFilters);
            DOCTEST_PRINTF_COLORED("[doctest] ", Color::Cyan);
            std::printf("test_catch2 suites with unskipped test_catch2 cases passing the current filters: %u\n",
            ...
            DOCTEST_SNPRINTF(buff, DOCTEST_COUNTOF(buff), "test_catch2 cases: %6u",
                             p->numTestsPassingFilters);
...
void Context::parseArgs(int argc, const char* const* argv, bool withDefaults) {
    ...
    parseCommaSepArgs(argc, argv, "dt-test_catch2-suite=",         p->filters[2]);
    parseCommaSepArgs(argc, argv, "dt-ts=",                 p->filters[2]);
    parseCommaSepArgs(argc, argv, "dt-test_catch2-suite-exclude=", p->filters[3]);
    parseCommaSepArgs(argc, argv, "dt-tse=",                p->filters[3]);
    parseCommaSepArgs(argc, argv, "dt-test_catch2-case=",          p->filters[4]);
    parseCommaSepArgs(argc, argv, "dt-tc=",                 p->filters[4]);
    parseCommaSepArgs(argc, argv, "dt-test_catch2-case-exclude=",  p->filters[5]);
...
#undef DOCTEST_PARSE_AS_BOOL_OR_FLAG
    ...
    if(parseFlag(argc, argv, "dt-list-test_catch2-cases") || parseFlag(argc, argv, "dt-ltc")) {
        p->list_test_cases = true;
        p->exit            = true;
    }
    if(parseFlag(argc, argv, "dt-list-test_catch2-suites") || parseFlag(argc, argv, "dt-lts")) {
...
// the main function that does all the filtering and test_catch2 running
int Context::run() {
    using namespace detail;
    ...
    if(p->list_test_cases) {
        DOCTEST_PRINTF_COLORED("[doctest] ", Color::Cyan);
        std::printf("listing all test_catch2 case names\n");
        DOCTEST_PRINTF_COLORED(getSeparator(), Color::Yellow);
    }
    ...
    if(p->list_test_suites) {
        DOCTEST_PRINTF_COLORED("[doctest] ", Color::Cyan);
        std::printf("listing all test_catch2 suites\n");
        DOCTEST_PRINTF_COLORED(getSeparator(), Color::Yellow);
    }
    ...
    for(i = 0; i < testArray.size(); i++) {
        ...
        // do not execute the test_catch2 if we are to only count the number of filter passing tests
        if(p->count)
            continue;

        // print the name of the test_catch2 and don't execute it
        if(p->list_test_cases) {
            std::printf("%s\n", data.m_name);
            continue;
        }

        // print the name of the test_catch2 suite if not done already and don't execute it
        if(p->list_test_suites) {
        ...
        // skip the test_catch2 if it is not in the execution range
        if((p->last < p->numTestsPassingFilters && p->first <= p->last) ||
           (p->first > p->numTestsPassingFilters))
            continue;

        // execute the test_catch2 if it passes all the filtering
        {
            p->currentTest = &data;
...
// execute the test_catch2
            ...
            if(failed) // if any subcase has failed - the whole test_catch2 case has failed
                p->numFailed++;
