...
void registerTestCaseFunction
    ...
    #define INTERNAL_CATCH_TEST_CASE_METHOD2( TestName, ClassName, ... )\
        ...
        namespace{ \
            ...
            Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME( autoRegistrar ) ( &TestName::test_catch2, #ClassName, Catch::NameAndDesc( __VA_ARGS__ ), CATCH_INTERNAL_LINEINFO ); /* NOLINT */ \
        } \
        CATCH_INTERNAL_UNSUPPRESS_ETD_WARNINGS \
        void TestName::test_catch2()
...
namespace Catch {
    ...
    struct ResultDisposition { enum Flags {
        ...
        ContinueOnFailure = 0x02,   // Failures fail test_catch2, but execution continues
        FalseTest = 0x04,           // Prefix expression with !
        SuppressFail = 0x08         // Failures are reported but do not fail the test_catch2
    }; };
...
namespace Detail {
    ...
    class IsStreamInsertable {
        ...
        static auto test_catch2(int)
        -> decltype( std::declval<SS&>() << std::declval<TT>(), std::true_type() );

        template<typename, typename>
        static auto test_catch2(...) -> std::false_type;

    public:
        static const bool value = decltype(test_catch2<std::ostream,const T&>(0))::value;
    };
#endif
...
// macros.
// This can potentially cause false negative, if the test_catch2 code catches
...
namespace Catch {
    ...
    inline Clara::CommandLine<ConfigData> makeCommandLineParser() {
        ...
        cli["-l"]["--list-tests"]
            .describe( "list all/matching test_catch2 cases" )
...
//            .placeholder( "level" );
        ...
        cli[_]
            .describe( "which test_catch2 or tests to use" )
            .bind( &addTestOrTags, "test_catch2 name, pattern or tags" );

        cli["-d"]["--durations"]
            .describe( "show test_catch2 durations" )
            .bind( &setShowDurations, "yes|no" );

        cli["-f"]["--input-file"]
            .describe( "load test_catch2 names to run from a file" )
        ...
        cli["--list-test_catch2-names-only"]
            .describe( "list all/matching test_catch2 cases names only" )
            .bind( &ConfigData::listTestNamesOnly );

        cli["--list-extra-info"]
            .describe( "list all/matching test_catch2 cases with more info" )
        ...
        cli["--order"]
            .describe( "test_catch2 case order (defaults to decl)" )
...
namespace Catch {
    ...
    inline std::size_t listTests( Config const& config ) {
        ...
        if( config.testSpec().hasFilters() )
            Catch::cout() << "Matching test_catch2 cases:\n";
        else {
            Catch::cout() << "All available test_catch2 cases:\n";
        ...
        if( !config.testSpec().hasFilters() )
            Catch::cout() << pluralise( matchedTests, "test_catch2 case" ) << '\n' << std::endl;
        else
            Catch::cout() << pluralise( matchedTests, "matching test_catch2 case" ) << '\n' << std::endl;
        return matchedTests;
    }
    ...
    inline std::size_t listTags( Config const& config ) {
        TestSpec testSpec = config.testSpec();
        if( config.testSpec().hasFilters() )
            Catch::cout() << "Tags for matching test_catch2 cases:\n";
        else {
...
namespace Catch {
    ...
    private: // IResultCapture
        ...
        virtual void handleFatalErrorCondition( std::string const& message ) {
            ...
            // Recreate section for test_catch2 case (as we will lose the one that was in scope)
    ...
    private:
        ...
        void runCurrentTest( std::string& redirectedCout, std::string& redirectedCerr ) {
            ...
            try {
                ...
                // This just means the test_catch2 was aborted due to failure
            }
            catch(...) {
...
#define CATCH_CONFIG_DEFAULT_REPORTER "console"
    ...
    void applyFilenamesAsTags( IConfig const& config ) {
        ...
        for(std::size_t i = 0; i < tests.size(); ++i ) {
            TestCase& test_catch2 = const_cast<TestCase&>( tests[i] );
            std::set<std::string> tags = test_catch2.tags;

            std::string filename = test_catch2.lineInfo.file;
...
namespace Catch {
    ...
    public:
        ...
        virtual void registerTest( TestCase const& testCase ) {
            ...
            if( name.empty() ) {
                std::ostringstream oss;
                oss << "Anonymous test_catch2 case " << ++m_unnamedCount;
...
namespace Catch {
    ...
    TestCase::TestCase( ITestCase* testCase, TestCaseInfo const& info ) : TestCaseInfo( info ), test_catch2( testCase ) {}

    TestCase::TestCase( TestCase const& other )
    :   TestCaseInfo( other ),
        test_catch2( other.test_catch2 )
    {}
    ...
    void TestCase::swap( TestCase& other ) {
        test_catch2.swap( other.test_catch2 );
    ...
    bool TestCase::operator == ( TestCase const& other ) const {
        return  test_catch2.get() == other.test_catch2.get() &&
                name == other.name &&
                className == other.className;
    }
...
namespace Catch {
    ...
    void ResultBuilder::react() {
        ...
        if (m_shouldDebugBreak) {
            ///////////////////////////////////////////////////////////////////
            // To inspect the state during test_catch2, you need to go one level up the callstack
            // To go back to the test_catch2 and change execution, jump over the throw statement
            ///////////////////////////////////////////////////////////////////
...
namespace Catch {
    ...
    public:
        ...
        static std::string getDescription() {
            return "Reports test_catch2 results as an XML document";
        }
...
namespace Catch {
    ...
    public:
        ...
        static std::string getDescription() {
            return "Reports test_catch2 results in an XML format that looks like Ant's junitreport target";
        }
        ...
        void writeTestCase( TestCaseNode const& testCaseNode ) {
            ...
            // All test_catch2 cases have exactly one section - which represents the
            // test_catch2 case itself. That section may have 0-n nested sections
...
namespace Catch {
    ...
    struct ConsoleReporter : StreamingReporterBase {
        ...
        static std::string getDescription() {
            return "Reports test_catch2 results as plain lines of text";
        }

        virtual void noMatchingTestCases( std::string const& spec ) CATCH_OVERRIDE {
            stream << "No test_catch2 cases matched '" << spec << '\'' << std::endl;
        }
        ...
        virtual void sectionEnded( SectionStats const& _sectionStats ) CATCH_OVERRIDE {
            if( _sectionStats.missingAssertions ) {
                ...
                if( m_sectionStack.size() > 1 )
                    ...
                    stream << "\nNo assertions in test_catch2 case";
        ...
        void printTestCaseAndSectionHeader() {
            ...
            if( m_sectionStack.size() > 1 ) {
                ...
                std::vector<SectionInfo>::const_iterator
                    it = m_sectionStack.begin()+1, // Skip first section (test_catch2 case)
        ...
        void printTotals( Totals const& totals ) {
            if( totals.testCases.total() == 0 ) {
                ...
                stream << " ("
                        ...
                        << pluralise( totals.testCases.passed, "test_catch2 case" ) << ')'
                        << '\n';
            }
            else {
                ...
                printSummaryRow( "test_catch2 cases", columns, 0 );
...
namespace Catch {
    ...
    struct CompactReporter : StreamingReporterBase {
        ...
        static std::string getDescription() {
            return "Reports test_catch2 results on a single line, suitable for IDEs";
        }
    ...
    private:
        ...
        // - white: No tests ran.
        // -   red: Failed [both/all] N test_catch2 cases, failed [both/all] M assertions.
        // - white: Passed [both/all] N test_catch2 cases (no assertions).
            ...
            if( totals.testCases.total() == 0 ) {
                ...
                stream <<
                    "Failed " << bothOrAll( totals.testCases.failed )
                              << pluralise( totals.testCases.failed, "test_catch2 case"  ) << ", "
                ...
                stream <<
                    "Passed " << bothOrAll( totals.testCases.total() )
                              << pluralise( totals.testCases.total(), "test_catch2 case" )
                              << " (no assertions).";
            }
            else if( totals.assertions.failed ) {
                Colour colour( Colour::ResultError );
                stream <<
                    "Failed " << pluralise( totals.testCases.failed, "test_catch2 case"  ) << ", "
